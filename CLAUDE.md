...
Ownership in short: pages written by `scripts/migrate_wiki.py` are fixed through its `PATCHES` / `PAGES` / `ALT_TEXT` tables (or frozen), not edited in place; `docs/index.md`, `start-here/*`, `about/*`, `course-design/instructor-materials.md`, `modules/module-1/{concept-quiz,diagnostic-survey}.md`, `guest-lectures/*` (except its generated `index.md`), and `docs/log.md` are hand-written; section `index.md` files, `about/wiki-crosswalk.md`, and `docs/llms*.txt` are generated. Never write a `verified:` key. Never link anything under `instructor/` from `docs/`.
...
.venv/bin/python scripts/check_consistency.py                      # labs agree with the pages describing them (not CI)
...
- **Guest lecture slides** (`docs/materials/guest-lectures/agentic-ai-loops.html`): a slide deck (43 slides since the 2026-09-22 cut: a 21-slide, thirty-minute talk, two backup sections and the references) exported as one self-contained viewer (fonts and images embedded as `data:` URIs, no requests; slides are `<section class="slide" id="...">` on a fixed 1920x1080 stage scaled to fit; speaker notes live in the `#notes-data` JSON, images in `#img-data`). Its bracketed citations are `<a href>` hyperlinks, which the lint allows; anything that loads a resource is still rejected. The PDF twin is `docs/assets/files/Agentic-AI-Loops-Guest-Lecture.pdf`, printed from the HTML (its print stylesheet puts one slide on each 1920x1080 page); `docs/guest-lectures/agentic-ai-loops.md` embeds both and repeats the reference list, so change all three together.
