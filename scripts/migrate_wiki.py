...
PATCHES: dict[str, list[tuple[str, str]]] = {
    ...
    "modules/module-2/chapter-quizzes.md": [
        ...
        ('C. Hallucination in Thought — after the calculator error, the agent inserted a population figure from parametric memory (not from any Observation) as if it were retrieved information; the combined failure involves both incorrect tool selection (root cause) and hallucination in Thought (secondary consequence).',
         'C. Hallucination in Thought — after the calculator error, the agent inserted a population figure from parametric memory (not from any tool result) as if it were retrieved information; the combined failure involves both incorrect tool selection (root cause) and hallucination in Thought (secondary consequence).'),
        ...
        ('> ❌ **C is partially correct** but incomplete as a primary classification. The hallucination in Thought is real — the agent inserts a specific figure (13.96 million) not grounded in any Observation. However, Reading Guide 3 structures the five failure classes by root cause, and the root cause here is incorrect tool selection, not hallucination. Identifying the root cause is what enables the structural fix — and fixing tool selection (providing a web search option with clear scope) resolves both failures simultaneously.',
         '> ❌ **C is partially correct** but incomplete as a primary classification. The hallucination is real — the agent asserts a specific figure (13.96 million) that no tool result contained. However, Reading Guide 3 structures the five failure classes by root cause, and the root cause here is incorrect tool selection, not hallucination. Identifying the root cause is what enables the structural fix — and fixing tool selection (providing a web search option with clear scope) resolves both failures simultaneously.'),
...
ALT_TEXT = {
    ...
    # Corrected after opening the file: it is a five-row table, not a loop diagram, and it uses
    # its own component names. The page text around it maps those onto the four the course teaches.
    "AgentExecutor-Architecture.png": "A five-row table titled Component, Role and Design Considerations, "
                                      "describing Agent (the LLM), Tool Descriptions, Tool Executor, Memory / State "
                                      "and Stopping Criteria for the LangChain 0.x AgentExecutor",
