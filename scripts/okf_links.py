...
to files served verbatim (`materials/`, `assets/`) resolve to the file
itself, and anchors, external URLs, `mailto:`, fragment-only links and paths
that are already absolute are left alone.

The source files under `docs/` are never modified: the site build needs the
relative links, and `site_lint.py` checks that they resolve.
"""
...
# Raw HTML attribute, for the handful of pages that embed an iframe or object.
...
def absolutize(target: str, source_rel: str, base: str) -> str:
    ...
    if resolved.startswith(".."):  # escapes docs/; leave it for the linter to flag
        return target
...
def rewrite_relative_targets(text: str, source_rel: str | Path, base: str) -> str:
    ...
    `source_rel` is the page's path relative to docs/ (e.g. `modules/module-1/overview.md`),
    `base` the site URL with a trailing slash. Fenced code blocks are left untouched, so
...
def _config() -> str:
    return (Path(__file__).resolve().parent.parent / "zensical.toml").read_text(encoding="utf-8")
